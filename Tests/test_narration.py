...
# <-- MODIFIED: Apply patch as a class decorator
@patch('utils.utilities.load_data') 
class TestNarrationManager(unittest.TestCase):
    """Unit tests for the NarrationManager in game/narration.py"""

    # <-- MODIFIED: Mock object is passed by the decorator
    def setUp(self): 
        ...
        # --- Mock Roles ---
        # Roles should load correctly now using the patched load_data
        self.townie_role = get_role_instance("Townie")
        self.mafia_role = get_role_instance("Godfather")
        self.jester_role = get_role_instance("Jester")
        # Verify roles loaded correctly
        self.assertIsNotNone(self.townie_role, "Townie role failed to load")
        self.assertIsNotNone(self.mafia_role, "Godfather role failed to load")
        self.assertIsNotNone(self.jester_role, "Jester role failed to load")
    ...
    def mock_load_data_side_effect(self, filepath, error_default=None):
        ...
        # Use filename matching to return specific mock data
        if "role_definition.json" in os.path.basename(filepath):
            return {
                "Townie": {"base": "Town", "description": "D", "short_description": "SD"},
                "Godfather": {"base": "Mafia", "description": "D", "short_description": "SD"},
                "Jester": {"base": "NeutralEvil", "description": "D", "short_description": "SD"},
            }
        # Add other file mocks if needed by NarrationManager setup or tests
    ...
    def test_story_kill(self):
        ...
        story = self.narration_manager.construct_story('pre-day', 1)
        self.assertIsNotNone(story) # Check story is generated now
        self.assertIn("**PlayerOne** was found dead", story)
        self.assertIn("victim of the **Mafia**", story) 
    ...
    def test_story_kill_immune_fixed(self):
        ...
        story = self.narration_manager.construct_story('pre-day', 1)
        self.assertIsNotNone(story) # Check story is generated now
        self.assertIn("ambushed **Townie**", story) 
        self.assertIn("attack had no effect!", story) 

    def test_story_kill_blocked(self):
        self.narration_manager.add_event('kill_blocked', killer=self.player2, target=self.player1)
        story = self.narration_manager.construct_story('pre-day', 1)
        self.assertIsNotNone(story) # Check story is generated now
        self.assertIn("A **Godfather** member tried to attack **PlayerOne**", story)
        self.assertIn("someone intervened and blocked them!", story)
    ...
    def test_story_investigation(self):
        self.narration_manager.add_event('investigation', investigator=self.player1, target=self.player2)
        story = self.narration_manager.construct_story('pre-day', 1)
        self.assertIsNotNone(story) # Check story is generated now
        self.assertIn("A curious **Townie** decided to investigate", story)
    ...
    def test_story_lynch_single_victim(self):
        ...
        # <-- MODIFIED: Match the exact phrasing from the traceback
    ...
    def test_story_lynch_multiple_victims(self):
        ...
        story = self.narration_manager.construct_story('pre-night', 1)
        self.assertIsNotNone(story) # Check story is generated now
        self.assertIn("**PlayerOne** (the **Town - Townie**)", story) 
