...
def handle_block(game, blocker_id, target_id, night_outcomes):
    ...
    """
    # --- Step 1: Get player objects for the narration ---
    ...
    if not blocker or not target:
        ...
        return
    # --- Step 2: Check if the blocker's own action was cancelled ---
    ...
    if target_action and target_action.get('status') is None:
        ...
        game.narration_manager.add_event(event_type, blocker=blocker, target=target)
        logger.info(f"event type: {event_type}: Action by {target.display_name} was blocked by {blocker.display_name}.")
    else:
        # --- MISSED BLOCK ---
        ...
        event_type = 'block_missed_royale' if game.game_settings.get('game_type') == "battle_royale" else 'block_missed'
        game.narration_manager.add_event(event_type, blocker=blocker, target=target)
        logger.info(f"event type: {event_type}: Block by {blocker.display_name} on {target.display_name} missed as the target's action had already resolved.")
...
def handle_kill(game, killer_id, victim_id, night_outcomes):
    ...
    logger.info(f"Processing kill attempt by {killer.display_name} on {victim.display_name}.")
    ...
    if killer_action and killer_action.get('status') == 'blocked':
        ...
        logger.info(f"event type kill_blocked: Kill attempt by {killer.display_name} failed because they were blocked.")
        return
    # --- Step 2b: Check if the Killer is already dead ---
    if not killer.is_alive:
        logger.info(f"Kill attempt by {killer.display_name} did not happen because the killer is dead.")
        event_type = 'kill_missed'  # Using 'kill_missed' to indicate the kill did not occur.
        if game.game_settings.get('game_type') == "battle_royale":
            event_type = 'kill_missed_battle_royale'
            game.narration_manager.add_event(event_type, killer=killer, target=victim)
        return 
    # --- Step 3: Mark the killer's action as used ---
    # The action is consumed regardless of whether the target is immune.
    if killer_action:
        logger.info(f"Kill action by {killer.display_name} is marked as used.")
    ...
    if victim.role and victim.role.is_night_immune:
        ...
        logger.info(f"event type: kill_immune by {killer.display_name} on {victim.display_name} failed due to immunity.")
        return
    ...
    logger.info(f"kill by {killer.display_name} on {victim.display_name} was logged as successful into successful kill list.")
...
def handle_investigation(game, investigator_id, target_id, night_outcomes):
    ...
    # --- Step 3b: Log the event for narration ---
